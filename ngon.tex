...
programming approach has been suggested by Oliveira and Vallentin \cite{mario2013computing}. Though the SDP method has not yet yielded a nontrivial
...
Note that, while the half-length parallelogram is not unique if $K$ is not strictly convex, any non-unique half-length parallelograms associated to a particular direction have at least one half-length chord contained in an edge of $K$ and are equivalent by sliding motions along of the chord along an edge of $K.$ Thus, any two half-length parallelograms in the direction $\theta$ have equal area, and we can define that area as a function $A(\theta)$ of the direction.  
...
From this, it is possible to determine an initial affine diameter in a particular direction.  This is done by extending a set of parallel rays from all the vertices into the interior of $K$ and selecting the longest.  However, it does not give the initial configuration of the vertices of the half-length parallelogram or the area thereof.

\subsection{Tracking the affine diameter }

In most situations, given an initial affine diameter in a particular direction, the affine diameter can be tracked by sweeping out the ray in a counter-clockwise direction from the initial vertex until it meets another vertex.  From there, it is possible that the affine diameter continues to extend from the original vertex or to begin to sweep out counter-clockwise from the new vertex.

The conditions for staying at the same vertex or switching are given exactly by the direction of the convex cone of the edges which the affine diameter is entering.  The chosen vertex is that which is closer to the vertex of the cone.  In the special case where the affine diameter meets parallel edges, It is not uniquely determined, exactly because this cone is degenerate.  However, in such cases, the non-unique behavior is restricted to sliding the affine diameter along the parallel edges, and as the affine diameter continues to rotate, it  will meet a new cone, although it may also be degenerate. 

%figures

In the packing, the affine diameter defines a column of $K.$
...
\begin{equation}
    \begin{aligned}
   & \alpha_1 = 0\\
&_2 = \frac{1}{2} \sin \left(\phi _3-\phi _4\right) \csc \left(\phi _2-\phi _3\right)\\
&_3 = \frac{1}{2} \sin
   \left(\phi _2-\phi _4\right) \csc \left(\phi _2-\phi _3\right)\\
  &_4=1\\
...
From this, it is clear that there are points which have rate constant $k_i=0$ only when on which 

Also, are degenerate exactly when there are two vertices of the half-parallelogram sharing an edge.  This is exactly when there is a free sliding motion that preserves area.


---comments on the conditions for when $k_i=0$


%normalizing the hight
...
\begin{equation}\label{eq:lin-hept}
    \text{minimize } c\cdot z \text{ subject to } G z\ge 0\text.
\end{equation}
Unfortunately, \ref{lin-hept} is unbounded. This can be shown by producing some
...
\begin{enumerate}
	...
	and the vertices of the corresponding affine diameter.
    \item The direction of the polygon edges on which the vertices above lie, except for
	the end of the affine diameter that lies on a polygon vertex.
    \item The distance and direction along the edge from those points to the nearest polygon vertex.
\end{enumerate}
