...
We prove the Kuperberg and Kuperberg double lattice construction for polygons is an isolated local maximal for density in the full space of packings.  That is, the density of nearby packings are of strictly lower density.
...
\begin{remark}
    ...
    That is, $\Lambda$ is a local maximum for density in the topological space $\Omega.$ 
\end{remark}
\begin{remark}
...
comment: restrict to saturated packings, fix $R$ large, 
...
We are considering the configuration space of four polygons in the plane, in a neighborhood of a Kuperberg and Kuperberg configuration.   Note that there are multiple choices of fundamental domains for the initial configuration, we choose the ...............

\subsubsection{ constraints }

Since we are working in the space of packings, the configuration space has a non-overlap constraint.  Locally, this can be described by a non-negativity condition on the signed volume of triangles defined by oriented triples of vertices.  

This requires a lemma to show that we are still exploring a full neighborhood....  (point point contacts in the polygons)  

\begin{lemma}
In a neighborhood of the Kuperberg Kuperberg rhomb, the non-negativity of the signed area condition is equivalent to the non-overlap condition 
\end{lemma}

picture of a possible violation of the lemma...
...
The lemma should be a consequence of the existence of the extensive parallelogram.  or is it true?
